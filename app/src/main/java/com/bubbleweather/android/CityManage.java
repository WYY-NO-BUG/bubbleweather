...
public class CityManage extends AppCompatActivity {
    ...
    private List<SaveDate> saveDataList;
    private List<SaveDate> selectList;
    private SaveDate deleteList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saveDataList = LitePal.findAll(SaveDate.class);
        if (saveDataList.size() > 0){
            for (SaveDate saveData:saveDataList
                 ) {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
                ...
                test.setPositiveButton("sure", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        LitePal.deleteAll(SaveDate.class,"csName = ?",deleteList.getCsName());
                        selectList = LitePal.findAll(SaveDate.class);
                        if (selectList.size() > 0){
                            dataList.clear();
                            for (SaveDate saveDate : selectList){
                                dataList.add(saveDate.csName);
                            }
                        }else {
                            dataList.clear();
                        }
